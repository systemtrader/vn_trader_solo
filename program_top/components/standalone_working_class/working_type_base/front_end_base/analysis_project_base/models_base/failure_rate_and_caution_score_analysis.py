...
class failure_rate_and_caution_score_analysis(models_base):
	...
	def __init__(self):
		...
		self._caution_base_number=int(self._config['caution_base'])**int(self._config['caution_base_exponent'])#警告指数函数的底
		self._caution_threshold=int(self._config['warning_threshold'])#警告指数函数报警的阈值
	...
	def __do_the_stats(self):
		...
		from program_top.components.standalone_working_class.working_type_base.backend_interface_base.s3_gate_uploader import s3_gate_uploader
