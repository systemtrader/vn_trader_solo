...
from program_top.standalone_working_class.working_type_base.backend_interface_base import backend_interface_base
...
class buffer_cleaning_up(backend_interface_base):
	...
	def __init__(self):
		...
		days_of_retention=int(self._config['day_of_retension'])
		date_today=datetime.now().date()
		self.oldest_retaining_day=date_today-timedelta(days=days_of_retention)#保留文件的最后日期
		repeating_day=int(self._config['repeating_day'])
